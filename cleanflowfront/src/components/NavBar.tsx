...
export const NavBar = () => {
  ...
  return (
    <main
      className={`flex flex-col border-[0.1px] border-border-accent rounded-xl m-5 h-[96vh] bg-bg-secondary text-white font-normal transition-all duration-300 ease-in-out overflow-hidden ${collapsed ? 'w-15' : 'w-50'}`}
    >
      <div className="border-b-[0.1px] border-b-border-accent h-15 w-full py-3 px-5 text-xl font-normal flex items-center justify-between">
        ...
        {routes.map(({ name, path, icon }) => {
          return (
            <NavLink
              ...
              to={path}
              className={({ isActive }) =>
                `h-10 text-xs flex items-center gap-2 p-2 rounded-lg ${
