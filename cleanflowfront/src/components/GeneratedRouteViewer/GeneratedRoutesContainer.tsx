...
export const GeneratedRoutesContainer = ({ driverId, date }: RoutesContainerProps) => {
  ...
  const {
    ...
    getNextPageParam: lastPage => {
      return lastPage.has_next ? lastPage.cursor : null
    },
    staleTime: 30_000,
    throwOnError: false,
    meta: {
  ...
  return (
    <div ref={rootRef} className="flex flex-col flex-1 gap-2 rounded-lg overflow-y-scroll">
      {paginatedGeneratedRoutes?.map(({ id, driver, created_at, polylines }) => {
        const polylineEnconded = polyline.encode(
          polylines.flatMap(p => polyline.decode(p.polyline) as [number, number][]),
        )
        return (
          <CardGeneratedRoute
            key={id}
      ...
      <div ref={bottomRef} className="py-1">
        {isFetchingNextPage && <p className="text-center text-sm text-gray-400">Cargando más...</p>}
      </div>
    </div>
  )
}
