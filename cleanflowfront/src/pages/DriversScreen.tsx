  ...
  return (
    <div className="h-[96vh] rounded-xl w-[97%] border-[0.1px] border-border-accent bg-bg-secondary flex flex-col p-6 overflow-hidden">
      <div className="flex items-center gap-3 mb-4">
        ...
        {!isLoading && (
          <span className="font-normal text-tiny text-white/50 bg-black/20 px-2 py-0.5 rounded-md border-[0.1px] border-border-accent">
            {totalElements} registrados
          </span>
        )}
        <button
          onClick={() => setShowForm(true)}
          className="font-normal ml-auto px-4 py-1.5 text-tiny rounded-lg bg-blue-600 hover:bg-blue-700 text-white transition-colors cursor-pointer"
        >
          + Agregar
        </button>
      </div>
              ...
              <tbody>
                {drivers.map(driver => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <button
                        onClick={() => statusMutation.mutate({ userId: driver.id, enabled: !driver.enabled })}
                        disabled={statusMutation.isPending}
                        className={`flex items-center gap-2 cursor-pointer disabled:opacity-50 transition-all ${
                          driver.enabled ? 'text-green-400' : 'text-red-400'
                        }`}
                      >
                        <span
                        ...
                        <span className="text-tiny font-medium">{driver.enabled ? 'Activo' : 'Inactivo'}</span>
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 font-normal">
          <form
