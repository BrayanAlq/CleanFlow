...
export const GeneratedRoutesScreen = () => {
  ...
  const [residentsVisible, setResidentsVisible] = useState(true)
  ...
  const driverColors = useMemo(() => {
    ...
    generatedRoutes?.forEach(gr => {
      if (!map.has(gr.driver.id)) {
        map.set(gr.driver.id, palette[map.size % palette.length])
      }
    })
    return map
  }, [generatedRoutes])
    ...
    <div className="h-[96vh] rounded-xl w-[97%] border-[0.1px] border-border-accent relative overflow-hidden">
      ...
      >
        ...
        {containers.map(({ id, latitude, longitude }) => {
          return (
            <Marker key={id} position={[latitude, longitude]} icon={trashIcon} opacity={containersVisible ? 1 : 0} />
          )
        })}
      ...
      <DriverContainer />
      <ToggleVisibility
        containerVisibility={containersVisible}
        residentVisibility={residentsVisible}
        onContainersClick={() => setContainersVisible(prev => !prev)}
        onResidentsClick={() => setResidentsVisible(prev => !prev)}
      />
    </div>
  )
}
